...
#define VERIFY_COMMISSIONING_STEP(result, msg) \
  if (!result || !result->status) { \
    return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, msg, result.error() }; \
  }
...
namespace lcl::zigbee::adapter {
  ...
  Errean<ZStackError> ZStackAdapter::initializeProcessor() {
    // TODO: Do this a few times
    // Ping and capabilities
    auto ping_response = network_processor.ping();
    if (!ping_response) {
      return ZStackError {
    ...
    auto version_result = network_processor.getVersion();
    if (!version_result) {
    ...
    auto nwkkey_response = network_processor.getMemoryAlignment();
  ...
  Errean<ZStackError> ZStackAdapter::beginCommissioning(
    ...
    if (network_options.pan_id == 0xFFFF) {
      return ZStackError { EC_ZSTACK_INVALID_PAN_ID, "PAN ID cannot be 0xFFFF" };
    }

    Logger::info("USER", "Deleting network block information");
    const auto nib_result = network_processor.deleteNetworkInformationBlock();
    if (!nib_result || !nib_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to clear the NIB", nib_result.error() };
    }

    const auto clear_result = clearAdapter();
    if (!clear_result) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to clear the adapter", clear_result };
    }

    const auto update_result = updateCommissioningItems(network_options);
    ...
    if (std::set<uint8_t>({ZNP_VERSION_ZSTACK_3x0, ZNP_VERSION_ZSTACK_30x}).contains(version->product_id)) {
      ...
      auto set_channel_result = network_processor.setBdbCommissioningChannel(true, toChannelMask(network_options.channel_list));
      if (!set_channel_result || !set_channel_result->status) {
        return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the primary BDB Commissioning channel set", set_channel_result };
      }

      Logger::info("USER", "Setting secondary BDB Commissioning channels");
      set_channel_result = network_processor.setBdbCommissioningChannel(false, std::set<ZnpChannelMask> {});
      if (!set_channel_result || !set_channel_result->status) {
        return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the secondary BDB Commissioning channel set", set_channel_result };
      }

      Logger::info("USER", "Starting BDB Commissioning commissioning");
      const auto start_result = network_processor.startBdbCommissioning(ZNP_COMMISSIONING_MODE_NETWORK_FORMATION);
  ...
  Errean<ZStackError> ZStackAdapter::updateCommissioningItems(const ZnpNetworkOptions &network_options) {
    Logger::info("USER", "Clearing startup options");
    const auto startup_result = network_processor.setStartupOptions(ZnpStartupOptions::normal());
    if (!startup_result || !startup_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set startup options.", startup_result };
    }

    Logger::info("USER", "Setting the logical type to coordinator");
    const auto logical_type_result = network_processor.setLogicalType(LOGICAL_TYPE_COORDINATOR);
    if (!logical_type_result || !logical_type_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set logical type.", logical_type_result };
    }

    Logger::info("USER", "Enabling ZDO direct callback");
    const auto zdo_direct_db_result = network_processor.setZdoDirectCallback(true);
    if (!zdo_direct_db_result || !zdo_direct_db_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to enable the ZDO direct callback.", zdo_direct_db_result };
    }

    Logger::info("USER", "Setting the channel lists");
    const auto channel_list_result = network_processor.setChannelList(toChannelMask(network_options.channel_list));
    if (!channel_list_result || !channel_list_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the channel list.", channel_list_result };
    }

    Logger::info("USER", "Setting the pan ID");
    const auto pan_id_result = network_processor.setPanID(network_options.pan_id);
    if (!pan_id_result || !pan_id_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the pan ID.", pan_id_result };
    }

    Logger::info("USER", "Setting the extended pan ID");
    const auto extended_pan_id_result = network_processor.setExtendedPanID(network_options.extended_pan_id);
    if (!extended_pan_id_result || !extended_pan_id_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the extended pan ID.", extended_pan_id_result };
    }

    Logger::info("USER", "Setting the APS use extended pan ID");
    const auto aps_use_extended_pan_id_result = network_processor.setApsUseExtendedPanID(network_options.extended_pan_id);
    if (!aps_use_extended_pan_id_result || !aps_use_extended_pan_id_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set the APS use extended pan ID.", aps_use_extended_pan_id_result };
    }

    Logger::info("USER", "Enabling preconfigured keys");
    const auto enabled_prec_keys_result = network_processor.setPreconfiguredKeysEnabled(network_options.network_key_distribute);
    if (!enabled_prec_keys_result || !enabled_prec_keys_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to enable preconfigured keys.", enabled_prec_keys_result };
    }

    Logger::info("USER", "Setting preconfigured keys");
    const auto preconfigured_keys_result = network_processor.setPreconfiguredKeys(network_options.network_key);
    if (!preconfigured_keys_result || !preconfigured_keys_result->status) {
      return ZStackError { EC_ZSTACK_COMMISSIONING_FAILED, "Failed to set preconfigured keys", preconfigured_keys_result.error() };
    }

    return true;
  }

  Errean<ZStackError> ZStackAdapter::clearAdapter() {
    auto startup_write_result = network_processor.setStartupOptions(ZnpStartupOptions::clearDeviceAndNetwork());
    if (!startup_write_result || !startup_write_result->status)
      return ZStackError { EC_ZSTACK_FAILURE_STATUS, "Failed to set startup options", startup_write_result.error() };

    const auto reset_result = network_processor.reset();
    if (!startup_write_result || !startup_write_result->status)
      return ZStackError { EC_ZSTACK_FAILURE_STATUS, "Failed to reset the adapter", startup_write_result.error() };

    startup_write_result = network_processor.setStartupOptions(ZnpStartupOptions::normal());
    if (!startup_write_result || !startup_write_result->status)
      return ZStackError { EC_ZSTACK_FAILURE_STATUS, "Failed to set startup options", startup_write_result.error() };
  ...
  Errean<ZStackError> ZStackAdapter::clearZigbeeNetworkProcessor() {
    auto write_response = network_processor.setStartupOptions(ZnpStartupOptions::clearDeviceAndNetwork());
    ...
    network_processor.reset();

    write_response = network_processor.setStartupOptions(ZnpStartupOptions::normal());
    if (!write_response || !write_response->status) {
      return ZStackError {
        EC_ZSTACK_FAILURE_STATUS,
        "Received a failure status while writing startup options.",
  ...
  Errean<AdapterError> ZStackAdapter::start() {
    if (auto connect_result = network_processor.connect(); !connect_result) {
      return connect_result;
    }

    if (const auto initialize_result = initializeProcessor(); !initialize_result) {
      return AdapterError {
